...
#define MS5802_I2C_ADDRESS    0x77 // or 0x76
...
//-------------------------------------------------
// Constructor
MS_5802::MS_5802(uint16_t Resolution) {
...
//-------------------------------------------------
boolean MS_5802::initializeMS_5802(boolean Verbose) {
    Wire.begin();
  ...
  // Read sensor coefficients
    for (int i = 0; i < 8; i++ ){
      ...
      Wire.beginTransmission(MS5802_I2C_ADDRESS);
      Wire.write(0xA0 + (i * 2));
      Wire.endTransmission();
      Wire.requestFrom(MS5802_I2C_ADDRESS, 2);
    ...
    unsigned char n_crc = MS_5802_CRC(sensorCoeffs); 
...
void MS_5802::readSensor() {
  ...
  // to read.
  if (_Resolution == 256){
    D1 = MS_5802_ADC(CMD_ADC_D1 + CMD_ADC_256); // read raw pressure
    D2 = MS_5802_ADC(CMD_ADC_D2 + CMD_ADC_256); // read raw temperature 
  } else if (_Resolution == 512) {
    D1 = MS_5802_ADC(CMD_ADC_D1 + CMD_ADC_512); // read raw pressure
    D2 = MS_5802_ADC(CMD_ADC_D2 + CMD_ADC_512); // read raw temperature   
  } else if (_Resolution == 1024) {
    D1 = MS_5802_ADC(CMD_ADC_D1 + CMD_ADC_1024); // read raw pressure
    D2 = MS_5802_ADC(CMD_ADC_D2 + CMD_ADC_1024); // read raw temperature
  } else if (_Resolution == 2048) {
    D1 = MS_5802_ADC(CMD_ADC_D1 + CMD_ADC_2048); // read raw pressure
    D2 = MS_5802_ADC(CMD_ADC_D2 + CMD_ADC_2048); // read raw temperature
  } else if (_Resolution == 4096) {
    D1 = MS_5802_ADC(CMD_ADC_D1 + CMD_ADC_4096); // read raw pressure
    D2 = MS_5802_ADC(CMD_ADC_D2 + CMD_ADC_4096); // read raw temperature
  }
...
//    TEMP = 2000 + ((int64_t)dT * sensorCoeffs[6]) / (int64_t)1<<23;
    ...
    // Do 2nd order temperature compensation (see pg 9 of MS5802 data sheet)
...
unsigned char MS_5802::MS_5802_CRC(unsigned int n_prom[]) {
...
unsigned long MS_5802::MS_5802_ADC(char commandADC) {
  ...
  Wire.beginTransmission(MS5802_I2C_ADDRESS);
    ...
    // See table on page 1 of the MS5802 data sheet showing response times of
